...
class KyutaiStreamingSTT(STTService):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._finalize_delay = float(os.getenv("KYUTAI_FINALIZE_DELAY_SEC", "0.12"))
        ...
        self._token_buffer = []  # Store token ids for accurate detokenization
        self._piece_buffer = []  # Human-readable pieces for logging/debugging
        ...
        if PUNCTUATION_AVAILABLE:
            try:
                ...
                logger.debug("Fast punctuation restoration enabled")
        ...
        logger.debug(f"Kyutai STT initialized: {hf_repo} with VAD={enable_vad}")
    ...
    def _normalize_spacing(self, text: str) -> str:
        if not text:
            return text
        # Only normalize multiple spaces, don't split words incorrectly
        normalized = re.sub(r'\s+', ' ', text)
        return normalized.strip()

    def _reset_text_buffers(self):
        """Clear cached token and piece buffers used for transcription assembly."""
        self._token_buffer = []
        self._piece_buffer = []

    def _decode_buffer(self) -> str:
        """Decode the current token buffer into human-readable text."""
        if not self._token_buffer:
            return ""

        try:
            decoded = self._text_tokenizer.decode(self._token_buffer)
        except Exception as decode_err:
            logger.debug(f"SentencePiece decode failed, falling back to piece join: {decode_err}")
            decoded = "".join(self._piece_buffer)

        # Only strip leading/trailing whitespace, preserve SentencePiece spacing
        return decoded.strip()

    def _init_kyutai_model(self):
        """Initialize the Kyutai streaming model."""
        try:
            logger.debug(f"Loading Kyutai streaming model: {self._hf_repo}")
            ...
            logger.debug(f"Mimi weights loaded from: {mimi_weights}")
            moshi_name = config_dict.get("moshi_name", "model.safetensors")
            moshi_weights = hf_hub_download(self._hf_repo, moshi_name)
            logger.debug(f"Moshi weights loaded from: {moshi_weights}")
            tokenizer_path = hf_hub_download(self._hf_repo, config_dict["tokenizer_name"])
            logger.debug(f"Tokenizer loaded from: {tokenizer_path}")

            # Initialize model with global MLX lock to prevent Metal threading conflicts
            with MLX_GLOBAL_LOCK:
                ...
                # Flush Metal commands after model loading
                mx.eval(mx.array([0]))
            ...
            logger.debug(f"Tokenizer vocab size: {self._text_tokenizer.get_piece_size()}")
            logger.debug(f"Tokenizer EOS token ID: {self._text_tokenizer.eos_id()}")
            logger.debug(f"Tokenizer BOS token ID: {self._text_tokenizer.bos_id()}")
            logger.debug(f"Tokenizer PAD token ID: {self._text_tokenizer.pad_id()}")

            # Check what token 3 actually is
            if self._text_tokenizer.get_piece_size() > 3:
                token_3_piece = self._text_tokenizer.id_to_piece(3)
                logger.debug(f"Token 3 represents: '{token_3_piece}'")
            ...
            logger.debug(f"EOS token IDs: {self._eos_token_ids}")
            logger.debug(f"PAD token ID: {self._pad_token_id}")
            logger.debug(f"BOS token ID: {self._bos_token_id}")
            ...
            if self._hf_repo.endswith("-candle"):
                ...
                logger.debug("Audio tokenizer backend: RustyMimi (Candle)")
            else:
                ...
                logger.debug("Audio tokenizer backend: moshi_mlx Mimi (MLX)")
    ...
    def _reset_model_state(self):
        ...
        try:
            ...
            self._reset_text_buffers()
            self._last_reset_time = time.time()

            logger.debug("Model state reset completed with fresh audio buffer")
            ...
            async with self._processing_lock:
                while len(self._audio_buffer) >= self._block_size:
                    ...
                    try:
                        ...
                        # Encode audio depending on tokenizer backend (with MLX lock)
                        with MLX_GLOBAL_LOCK:
                            ...
                            mx.synchronize()  # Flush Metal commands
                        ...
                        # Generate text token (with MLX lock)
                        with MLX_GLOBAL_LOCK:
                            if self._enable_vad:
                        ...
                        if not is_eos_token and not is_pad_token and not is_bos_token and not is_unk_token:
                            ...
                            text = text.replace("▁", " ")
                            logger.debug(f"Kyutai STT: '{text}' (token: {text_token})")

                            if text.strip():
                                # Add to buffers
                                self._token_buffer.append(text_token)
                                self._piece_buffer.append(text)
                                self._last_speech_time = len(block) / self._target_sample_rate
                                logger.debug(
                                    f"Added to text buffer: '{text}', buffer length: {len(self._token_buffer)}"
                                )

                                # Yield interim transcription with accumulated text every few tokens
                                if len(self._token_buffer) % 8 == 0:  # Every 8 tokens for more natural chunks
                                    accumulated_text = self._decode_buffer()
                            ...
                            if self._token_buffer and self._consecutive_eos_count >= 1:
    ...
    async def process_frame(self, frame: Frame, direction=None):
        ...
        if isinstance(frame, UserStartedSpeakingFrame):
            ...
            logger.debug("🎤 VAD detected user started speaking - clearing text buffer")
            self._reset_text_buffers()
            self._consecutive_eos_count = 0
            self._consecutive_pad_count = 0
        ...
        if isinstance(frame, UserStoppedSpeakingFrame):
            ...
            if since_last < 0.15:
                ...
                return
            logger.debug("🛑 VAD detected user stopped speaking - finalizing transcription (fallback)")
    ...
    async def flush(self) -> AsyncGenerator[Frame, None]:
        ...
        try:
            ...
            if self._token_buffer:
                final_text = self._decode_buffer()
                if final_text:
                    punctuated_text = self._add_punctuation(final_text)
                    logger.debug(
                        f"Finalizing buffered transcription: '{final_text}' -> '{punctuated_text}'"
                    )
