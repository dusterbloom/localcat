...
class TTSMLXIsolated(TTSService):
    ...
    def _start_worker(self):
        ...
        try:
            self._process = subprocess.Popen(
                ...
                stderr=subprocess.PIPE,  # Enable stderr capture
                text=True,
                bufsize=0,
            )
    ...
    def _send_command(self, command: dict) -> dict:
        ...
        try:
            ...
            # Increase timeout for model downloading (first run can take 30+ seconds)
            timeout = 60.0  # 60 second timeout for model initialization
            ready, _, _ = select.select([self._process.stdout], [], [], timeout)

            if not ready:
                return {"error": f"Worker response timeout after {timeout} seconds (model may be downloading)"}
