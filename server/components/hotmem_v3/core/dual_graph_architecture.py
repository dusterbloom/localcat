...
class DualGraphArchitecture:
    ...
    def configure(self, max_working_entities: int = 100, max_long_term_entities: int = 10000, 
                 promotion_interval: int = 1800):
        """Configure the dual graph architecture"""
        self.working_memory.max_entities = max_working_entities
        self.long_term_memory.max_entities = max_long_term_entities
        self.promotion_interval = promotion_interval
        
        logger.info(f"Dual graph configured: working={max_working_entities}, long_term={max_long_term_entities}")
    ...
    def get_combined_graph(self) -> Dict[str, Any]:
        """Get combined knowledge graph from both memory systems"""
        
        working_graph = self.working_memory.export_graph()
        long_term_graph = self.long_term_memory.export_graph()
        
        return {
            'entities': working_graph.get('entities', []) + long_term_graph.get('entities', []),
            'relations': working_graph.get('relations', []) + long_term_graph.get('relations', []),
            'metadata': {
                'working_memory_entities': len(working_graph.get('entities', [])),
                'long_term_memory_entities': len(long_term_graph.get('entities', [])),
                'total_entities': len(working_graph.get('entities', [])) + len(long_term_graph.get('entities', [])),
                'working_memory_relations': len(working_graph.get('relations', [])),
                'long_term_memory_relations': len(long_term_graph.get('relations', [])),
                'total_relations': len(working_graph.get('relations', [])) + len(long_term_graph.get('relations', [])),
                'timestamp': time.time()
            }
        }
