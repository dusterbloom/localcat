...
class ProfessionalKokoroTTSService(TTSService):
    ...
    def _ensure_models_downloaded(self):
        ...
        # Priority 1: Check bundled location (for Tauri builds)
        # Models are bundled at server/models/kokoro/ relative to this file
        bundled_dir = Path(__file__).parent.parent.parent / "models" / "kokoro"
        bundled_model = bundled_dir / "kokoro-v1.0.onnx"
        bundled_voices = bundled_dir / "voices-v1.0.bin"

        # Check if bundled files exist and have reasonable size
        try:
            bundled_model_exists = bundled_model.exists() and bundled_model.stat().st_size > 300_000_000
            bundled_voices_exists = bundled_voices.exists() and bundled_voices.stat().st_size > 25_000_000

            if bundled_model_exists and bundled_voices_exists:
                logger.info(f"📦 Using bundled Kokoro models from {bundled_dir}")
                return str(bundled_model.resolve()), str(bundled_voices.resolve())
        except Exception as e:
            logger.debug(f"Bundled models not found: {e}")

        # Priority 2: Check user cache (for development)
        ...
        if model_exists and voices_exists:
            logger.info(f"💾 Using cached Kokoro models from {cache_dir}")
    ...
    def _initialize_pipeline(self):
        ...
        try:
            ...
            self._pipeline = Kokoro(
                ...
                espeak_config=None  # Use internal phonemizer, not espeakng-loader
            )

            logger.info("🗣️  Using Kokoro internal phonemizer (no espeak-ng required)")

            logger.debug(f"✅ Professional Kokoro pipeline loaded")

            # Skip voice validation test in production to avoid espeak-ng issues
            # The test calls .create() which triggers espeakng-loader with hardcoded paths
            # In Tauri bundles, we trust that bundled models are correct
            skip_test = os.getenv("SKIP_TTS_VALIDATION", "false").lower() == "true"

            if not skip_test:
                # Test the voice with quality validation (dev environment only)
                try:
                ...
                logger.info(f"⏭️  Skipped voice validation test (SKIP_TTS_VALIDATION=true)")
    ...
    def _generate_audio_sync(self, text: str) -> Optional[tuple[np.ndarray, int, dict, dict]]:
        ...
        with self._tts_lock:
            try:
                ...
                logger.debug(f"🔧 About to call pipeline.create() for: '{text[:50]}...'")
                ...
                logger.debug(f"✅ pipeline.create() returned successfully")
    ...
    async def run_tts(self, text: str) -> AsyncGenerator[Frame, None]:
        ...
        # Use ultra-low latency chunking for optimal performance (250 chars per chunk)
        sentences = chunk_for_kokoro_ultra_low_latency(text, max_chars=250)
