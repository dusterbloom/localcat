    ...
    def create_intro_aware_pipeline(
        ...
        async def _debug_before_memory(frame) -> bool:
            ...
            if isinstance(frame, TranscriptionFrame):
                ...
                logger.critical(f"🔍 [PIPELINE IN] TranscriptionFrame id={fid} text='{frame.text[:50]}...' (seen {frame_tracker[fid]['count']}x)")
            return True
        ...
        async def _debug_before_llm(frame) -> bool:
            ...
            if isinstance(frame, TranscriptionFrame):
                logger.critical(f"🔍 [LLM INPUT] id={frame.id} text='{frame.text[:50]}...'")
            return True
        ...
        async def _debug_after_llm(frame) -> bool:
            ...
            from pipecat.frames.frames import TextFrame
            if isinstance(frame, TextFrame):
                logger.critical(f"🔍 [LLM OUTPUT] id={frame.id} text='{frame.text[:50]}...'")
            return True
        ...
        async def _debug_before_tts(frame) -> bool:
            ...
            from pipecat.frames.frames import TextFrame
            if isinstance(frame, TextFrame):
                logger.critical(f"🔍 [TTS INPUT] id={frame.id} text='{frame.text[:50]}...'")
            return True
